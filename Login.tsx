...
const Login: React.FC<LoginProps> = ({ onSwitchToSignup }) => {
  ...
  return (
    <div className="w-full bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg space-y-6">
      <h2 className="text-2xl font-bold text-center text-gray-900 dark:text-gray-100">{t('loginTitle')}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          >
            {isLoading ? <SpinnerIcon className="w-5 h-5" /> : t('loginButton')}
          </button>
        </div>
      </form>
      <p className="text-center text-sm text-gray-600 dark:text-gray-400">
        <button onClick={onSwitchToSignup} className="font-medium text-violet-600 hover:text-violet-500 dark:text-violet-400 dark:hover:text-violet-300">
          {t('switchToSignup')}
        </button>
      </p>
    </div>
